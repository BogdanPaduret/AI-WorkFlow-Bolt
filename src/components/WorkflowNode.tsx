...
function WorkflowNode({ id, data }: NodeProps) {
  const d = (data ?? {}) as Partial<WorkflowNodeData>;

  const nodeType = d.nodeType ?? 'prompt';

  const def = NODE_TYPES[nodeType] ?? NODE_TYPES.prompt;
  const Icon = def.icon;
  ...
  const bodyValue =
    nodeType === 'output'
      ? ((config as Partial<OutputConfig>).output ?? '')
      : ((config as Partial<PromptConfig>).text ?? '');
  ...
  return (
    <div className="wf-node" onDoubleClick={startEditBody}>
      ...
      >
        <GripVertical
          size={14}
          className="wf-drag-handle react-flow__drag-handle"
        />

        <div
          className="wf-node-type-icon"
      ...
      <div className="wf-node-body">
        {editingBody ? (
          <textarea
            ...
            onChange={(e) =>
              updateConfig({ [bodyKey]: e.target.value })
            }
          ...
          <p
            className="wf-node-text"
            style={{
              color: bodyValue ? '#e4e4e7' : '#3f3f46',
            }}
          >
